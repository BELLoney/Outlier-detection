...
%%%%%%%%%%%%%%%%% 计算第l个条件属性的邻域集合
LA=1:m-1; %属性集合序号
weight1=zeros(n,m-1);%单属性权重
weight3=zeros(n,m-1);%单属性权重
%Acc_A_a=zeros(n,m-1);%去掉一个属性之后的近似精度
% delta=[0.1 0 0.1]; %自己赋值delta
delta=zeros(1,m-1);%初始化邻域半径  
for j=1:m-1
...
for l=1:m-1
lA_d=setdiff(LA,l);
eval(['Acc_A_a' num2str(l) '=zeros(n,m-1);'])%去掉一个属性之后的近似精度
...
for i=1:n
 for k=1:m-1
     ...
     FSDOG(i,k)=1-((sum((Acc_A_a(i,2:m-1)+1)./2)+Acc_A_a(i,1))/(m-1))*weight1(i,k);  
  end 
end
 FRGOF=zeros(n,2);
for j=1:n
   FRGOF_temp=[];
   FRGOF_temp=sum(FSDOG(j,:).*weight3(j,:))/(m-1);
